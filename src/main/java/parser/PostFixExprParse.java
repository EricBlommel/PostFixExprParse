...
public class PostFixExprParse implements PostFixExprParseConstants {
    ...
    final public void varZuw(Token ident) throws ParseException {
        Token number;
        switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
            case EQUAL: {
                jj_consume_token(EQUAL);
    ...
    final public int condition() throws ParseException {
        ...
        switch (op.kind) {
            case EQ:
                jumpPlaceholder = code.if_icmpne();
                break; // a==b  -> springe, wenn a!=b
            case NE:
                jumpPlaceholder = code.if_icmpeq();
                break; // a!=b  -> springe, wenn a==b
            case LT:
                jumpPlaceholder = code.if_icmpge();
                break; // a<b   -> springe, wenn a>=b
            case GE:
                jumpPlaceholder = code.if_icmplt();
                break; // a>=b  -> springe, wenn a<b
            case GT:
                jumpPlaceholder = code.if_icmple();
                break; // a>b   -> springe, wenn a<=b
            case LE:
                jumpPlaceholder = code.if_icmpgt();
                break; // a<=b  -> springe, wenn a>b
            default: {
                if (true) throw new ParseException("wrong condition token");
            }
        }
        {
            {
    ...
    final public void statement() throws ParseException, LWertException {
        Token ident;
        int falseJump;
        int endJump;
        int loopStart;
...
// Mit GOTO zurück zum Schleifenanfang (per Platzhalter + Patch)
    ...
    final public void optElse(int falseJump) throws ParseException, LWertException {
